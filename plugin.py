...
Wersja 2.4 (poprawiona – GUI restart po aktualizacji)
"""
...
def get_s4aupdater_lists_dynamic():
    ...
    try:
        ...
        for var_name, url_value in urls_dict.items():
            ...
            date_info = versions_dict.get(version_key, "no date")
            lists.append(("{} - {}".format(display_name_base, date_info), "archive:{}".format(url_value)))
    except Exception as e:
        print("[AIO Panel] Error parsing S4aUpdater list:", e)
        return []
    return lists
...
# === DEFINICJE MENU ===
E2IPLAYER_INSTALL_CMD = "bash -c 'if ! opkg list-installed | grep -q ppanel; then echo \"Installing PPanel dependency...\"; wget http://downloads.openpli.org/feeds/openpli-9-release/3rd-party/ppanel_2.2_all.ipk -P /tmp && opkg install /tmp/ppanel_2.2_all.ipk; fi && echo \"Running main E2iPlayer installer...\"; wget -q \"https://raw.githubusercontent.com/oe-mirrors/e2iplayer/refs/heads/python3/e2iplayer_install.sh\" -O - | /bin/sh'"
...
# Helper: semantic version compare (major.minor[.patch])
def version_tuple(v):
...
class WizardProgressScreen(Screen):
    ...
    def __init__(self, session, steps, **kwargs):
        ...
        self["message"] = Label("Automatic installation in progress...\nPlease wait.\n\nDo not turn off the receiver.\nGUI will restart automatically when finished.")
    ...
    def _wizard_run_next_step(self, *args, **kwargs):
        ...
        if func_to_run:
            ...
            print("[AIO Panel] Unknown wizard step:", next_step)
            self._wizard_run_next_step()
    ...
    def _wizard_step_deps(self):
        title = self._get_wizard_title("Installing dependencies")
    ...
    def _wizard_step_channel_list(self):
        title = self._get_wizard_title("Installing channel list '{}'".format(self.wizard_channel_list_name))
    ...
    def _wizard_step_install_oscam(self):
        title = self._get_wizard_title("Installing Softcam Feed + Oscam")
        cmd = """
            echo "Installing/Updating Softcam Feed..."
            wget -O - -q http://updates.mynonpublic.com/oea/feed | bash
            echo "Updating package list..."
            opkg update
            echo "Searching for best Oscam version in feeds..."
            ...
            echo "Oscam installation finished."
            sleep 3
        """
        console_screen_open(self.session, title, [cmd], callback=self._wizard_run_next_step, close_on_finish=True)

    def _wizard_step_picons(self):
        title = self._get_wizard_title("Installing Picons (Transparent)")
    ...
    def _wizard_step_reload_settings(self):
        try:
            ...
            print("[AIO Panel] Error reloading settings in wizard:", e)
        self._wizard_run_next_step()

    def _on_wizard_finish(self, *args, **kwargs):
        self["message"].setText("Installation finished!\n\nGUI will restart in a moment...")
...
class Panel(Screen):
    ...
    def check_dependencies(self):
        ...
        if not missing_packages:
            ...
            return
        install_cmds = [
            "echo 'Missing packages detected. Installing automatically...'",
            "opkg update",
            "opkg install " + " ".join(missing_packages),
            "echo 'Dependencies installed! GUI restart recommended.'",
            "sleep 5"
        ]
    ...
    def fetch_update_info_in_background(self):
        try:
            ...
            print("[AIO Panel] Automatic update check error:", e)
    ...
    def on_update_finished(self, *args):
        # BEZ MessageBox – od razu restart GUI
    ...
    def _wizard_start(self, steps):
        channel_list_url, list_name, picon_url = '', 'default list', ''
        if "channel_list" in steps:
            ...
            if not channel_list_url:
                self.sess.open(MessageBox, "Could not fetch channel list URL.", type=MessageBox.TYPE_ERROR)
                return
        if "picons" in steps:
            for name, action in (TOOLS_AND_ADDONS_PL):
                if name.startswith("Pobierz Picony"):
                    try:
                        ...
                        picon_url = ''
            if not picon_url:
                self.sess.open(MessageBox, "Could not find picons URL.", type=MessageBox.TYPE_ERROR)
                return
    ...
    def execute_action(self, name, action):
        ...
        if action.startswith("bash_raw:"):
            ...
            if command_key == "SUPER_SETUP_WIZARD":
                self.run_super_setup_wizard()
            elif command_key == "CHECK_FOR_UPDATES":
                self.check_for_updates_manual()
            elif command_key == "NETWORK_DIAGNOSTICS":
                ...
                self.install_best_oscam(callback=lambda: self.ask_for_restart("Oscam installation finished.\nGUI restart recommended.\n\nRestart now?"), close_on_finish=True)
            elif command_key == "MANAGE_DVBAPI":
                self.manage_dvbapi()
            elif command_key == "UNINSTALL_MANAGER":
                self.show_uninstall_manager()
            elif command_key == "CLEAR_OSCAM_PASS":
                self.clear_oscam_password()
            elif command_key == "CLEAR_FTP_PASS":
                self.clear_ftp_password()
            elif command_key == "SET_SYSTEM_PASSWORD":
                self.set_system_password()
            elif command_key == "RESTART_OSCAM":
                self.restart_oscam()
            elif command_key == "FREE_SPACE_DISPLAY":
                self.show_free_space()
            elif command_key == "CLEAR_TMP_CACHE":
                console_screen_open(self.sess, title, ["rm -rf " + PLUGIN_TMP_PATH + "*"], close_on_finish=True)
            elif command_key == "CLEAR_RAM_CACHE":
                console_screen_open(self.sess, title, ["sync; echo 3 > /proc/sys/vm/drop_caches"], close_on_finish=True)
    ...
    def run_network_diagnostics(self):
        ...
        try:
            ...
            print("[AIO Panel] Error getting local IP:", e)
        ...
        cmd = """
            echo "--- AIO Panel - Network Diagnostics ---"
            echo " "
            ...
            echo "Running speed test (may take a minute)..."
            ...
            echo "Press OK or EXIT to close."
        """.format(
    ...
    def reload_settings_python(self, *args):
        try:
            ...
            db.reloadBouquets()
            show_message_compat(self.sess, "Channel list reloaded.", timeout=5)
        except Exception as e:
            print("[AIO Panel] Error reloading settings:", e)
            show_message_compat(self.sess, "Error reloading channel list.", message_type=MessageBox.TYPE_ERROR)
            ...
            for d in config_dirs:
                ...
                if fileExists(conf_path):
                    ...
                    if len(new_lines) < len(lines):
                        ...
                        found = True
            if found:
                show_message_compat(self.sess, "Oscam password cleared.")
            else:
                show_message_compat(self.sess, "Oscam password not found.")
        except Exception as e:
            show_message_compat(self.sess, "Error: {}".format(e), message_type=MessageBox.TYPE_ERROR)
    ...
    def manage_dvbapi(self):
        dvbapi_options = [
            ...
            ("Download from custom URL...", "custom"),
            ("Clear oscam.dvbapi file", "clear")
        ]
        self.sess.openWithCallback(
            self.on_dvbapi_option_selected,
            ChoiceBox,
            title="Manage oscam.dvbapi file",
    ...
    def on_dvbapi_option_selected(self, choice):
        if not choice:
            return
        if choice[1] == "custom":
            self.sess.openWithCallback(self.on_custom_dvbapi_url_entered, InputBox, title="Enter custom URL for oscam.dvbapi:", text="https://")
        elif choice[1] == "clear":
            self.sess.openWithCallback(self.do_clear_dvbapi, MessageBox, "Clear oscam.dvbapi file?", type=MessageBox.TYPE_YESNO)
        else:
    ...
    def process_dvbapi_download(self, url):
        ...
        console_screen_open(self.sess, "Updating oscam.dvbapi", [cmd], close_on_finish=True)
    ...
    def do_clear_dvbapi(self, confirmed):
        if confirmed:
            ...
            console_screen_open(self.sess, "Clearing oscam.dvbapi", [cmd], close_on_finish=True)

    def clear_ftp_password(self):
        console_screen_open(self.sess, "Clearing FTP password", ["passwd -d root"], close_on_finish=True)

    def set_system_password(self):
        self.sess.openWithCallback(lambda p: console_screen_open(self.sess, "Setting Password", ["(echo {}; echo {}) | passwd".format(p, p)], close_on_finish=True) if p else None, InputBox, title="Enter new root password:")

    def show_free_space(self):
        console_screen_open(self.sess, "Free space", ["df -h"], close_on_finish=False)

    def restart_oscam(self):
        cmd = 'for SCRIPT in softcam.oscam oscam softcam; do INIT_SCRIPT="/etc/init.d/$SCRIPT"; if [ -f "$INIT_SCRIPT" ]; then $INIT_SCRIPT restart; FOUND=1; break; fi; done; [ $FOUND -ne 1 ] && echo "Oscam startup script not found."; sleep 2;'
    ...
    def show_uninstall_manager(self):
        try:
            ...
            def on_package_selected(choice):
                if choice:
                    self.sess.openWithCallback(lambda c: console_screen_open(self.sess, "Uninstalling: " + choice[0], ["opkg remove " + choice[0]], close_on_finish=True) if c else None, MessageBox, "Uninstall package:\n{}?".format(choice[0]), type=MessageBox.TYPE_YESNO)
            self.sess.open(ChoiceBox, title="Select package to uninstall", list=[(p,) for p in packages])
        except Exception as e:
    ...
    def install_best_oscam(self, callback=None, close_on_finish=False):
        cmd = """
            ...
            sleep 3
        """
        console_screen_open(self.sess, "Oscam Installer", [cmd], callback=callback, close_on_finish=close_on_finish)
    ...
    def get_lists_from_repo(self):
        ...
        try:
            ...
            if not (os.path.exists(tmp_json_path) and os.path.getsize(tmp_json_path) > 0):
                print("[AIO Panel] Error downloading manifest.json: file empty or missing")
                raise IOError("Downloaded manifest file is empty or missing")
        except Exception as e:
            print("[AIO Panel] Error downloading manifest.json (exception):", e)
            raise
        lists_menu = []
        try:
            ...
            print("[AIO Panel] Error processing manifest.json:", e)
            raise
        if not lists_menu:
            print("[AIO Panel] No lists in repository (manifest empty?)")
            return []
        return lists_menu
