#!/bin/sh
# AIO Panel 16.0.0 - trusted MyUpdater v5.1 installer.
# Upstream equivalent: wget -q -O - URL | sh
...
URL='https://raw.githubusercontent.com/OliOli2013/MyUpdater-Plugin/main/installer.sh'
STATUS="${1:-/tmp/PanelAIO/myupdater.status}"
TMPROOT='/tmp/PanelAIO'; FILE="$TMPROOT/myupdater_installer_$$.sh"; LOG='/tmp/PanelAIO/logs/myupdater.log'
mkdir -p "$TMPROOT" "$(dirname "$STATUS")" "$(dirname "$LOG")" 2>/dev/null || true
rm -f "$FILE" "$STATUS" "$STATUS.tmp" 2>/dev/null || true
: > "$LOG" 2>/dev/null || true
log(){ printf '%s\n' "[AIO MyUpdater] $*" | tee -a "$LOG"; }
...
log 'Pobieranie MyUpdater v5.1 z zaufanego źródła OliOli2013/MyUpdater-Plugin...'
# Keep the exact wget options requested for this installer, but write to a local file.
wget -q -O "$FILE" "$URL" || fail 'Nie udało się pobrać instalatora MyUpdater.' download
...
"$PY" "$PLUGIN_DIR/core/remote_script_validator.py" "$FILE" myupdater >> "$LOG" 2>&1 || fail 'Instalator MyUpdater nie przeszedł walidacji AIO.' validation
HASH=$(aio_sha256 "$FILE" 2>/dev/null || true); log "SHA-256: $HASH"
chmod 700 "$FILE" || fail 'Nie można ustawić praw instalatora.' execute
(umask 022; PATH=/usr/sbin:/usr/bin:/sbin:/bin; export PATH; /bin/sh "$FILE") >> "$LOG" 2>&1 || fail 'Instalator MyUpdater zakończył się błędem.' execute
status "OK|sha256=$HASH|source=$URL|log=$LOG"
log 'Instalacja MyUpdater zakończona poprawnie.'
