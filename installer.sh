#!/bin/sh
# AIO Panel 15.0.0 - transactional GitHub source installer.
set -u
...
for REQUIRED in plugin.py legacy_plugin.py version.txt BUILD_ID.txt install_archive_script.sh picon_install_script.sh aio_safe_common.sh ui/modern.py ui/screens/connect.py assets/modern/qr_site.png; do [ -s "$NEW/$REQUIRED" ] || fail "Missing required file: $REQUIRED"; done
PY=$(aio_python 2>/dev/null || true); [ -n "$PY" ] || fail "Python interpreter not found."
"$PY" -m py_compile "$NEW/plugin.py" "$NEW/legacy_plugin.py" "$NEW/ui/modern.py" "$NEW/ui/screens/connect.py" >> "$LOG" 2>&1 || fail "Python syntax validation failed."
...
grep -q "build r2" "$DST/legacy_plugin.py" || { rm -rf "$DST" 2>/dev/null || true; [ -d "$BAK" ] && mv "$BAK" "$DST" 2>/dev/null || true; fail "Corrected build marker missing."; }
