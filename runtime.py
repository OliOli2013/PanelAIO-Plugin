...
if config is not None:
    try:
        ...
        if not hasattr(config.plugins.panelaio, 'update_channel'):
...
try:
    from Plugins.SystemPlugins.PanelAIO.core.logger import get_logger as _get_aio_logger, log_exception as _log_exception
    from Plugins.SystemPlugins.PanelAIO.core.result import normalize_result as _normalize_action_result
    from Plugins.SystemPlugins.PanelAIO.core.action_registry import requires_confirmation as _action_requires_confirmation, should_record_recent as _should_record_recent
    from Plugins.SystemPlugins.PanelAIO.core.action_dispatcher import dispatch as _dispatch_modular_action
    from Plugins.SystemPlugins.PanelAIO.core.activity import get_favorites as _get_favorites, get_recent as _get_recent, toggle_favorite as _toggle_favorite, record_recent as _record_recent
    from Plugins.SystemPlugins.PanelAIO.core.source_registry import is_trusted_remote_script as _is_trusted_remote_script, health_targets as _source_health_targets
except Exception:
    from core.logger import get_logger as _get_aio_logger, log_exception as _log_exception
    from core.result import normalize_result as _normalize_action_result
    from core.action_registry import requires_confirmation as _action_requires_confirmation, should_record_recent as _should_record_recent
    from core.action_dispatcher import dispatch as _dispatch_modular_action
    from core.activity import get_favorites as _get_favorites, get_recent as _get_recent, toggle_favorite as _toggle_favorite, record_recent as _record_recent
    from core.source_registry import is_trusted_remote_script as _is_trusted_remote_script, health_targets as _source_health_targets

AIO_LOGGER = _get_aio_logger()
try:
...
def _apply_auto_ram_from_config():
    """Restore automatic AIO maintenance setting after GUI/system restart."""
    ...
    try:
        ...
        if val and val != "off":
            ...
            print("[AIO Panel] Automatic AIO maintenance restored: {} min".format(minutes))
        else:
    ...
    def on_finish_thread():
        ...
        try:
            if result.get('success'):
                AIO_LOGGER.info('TASK success | %s | rc=%s', ensure_unicode(title), result.get('returncode', 0))
            else:
                AIO_LOGGER.error('TASK failed | %s | rc=%s | %s', ensure_unicode(title), result.get('returncode', '?'), ensure_unicode(result.get('stderr', ''))[-1200:])
        except Exception:
            pass
...
def _get_aio_tips(lang="PL"):
    ...
    tips_pl = [
        ...
        "Automatyczną konserwację AIO ustaw głównie wtedy, gdy chcesz okresowo usuwać stare pliki robocze AIO i uruchamiać porządkowanie pamięci Pythona.",
    ...
    tips_en = [
        ...
        "Use automatic AIO maintenance when you want periodic cleanup of stale AIO work files and Python memory collection.",
...
SOFTCAM_AND_PLUGINS_PL = [
    ...
    ("🔄 S4aUpdater - Instalator", "CMD:INSTALL_S4AUPDATER"),
    ...
    ("🔄 MyUpdater v5.1 - Instalator", "CMD:INSTALL_MYUPDATER"),
    ...
    ("🖼️ Picon Updater - Instalator (Picony)", "CMD:INSTALL_PICON_UPDATER"),
...
SOFTCAM_AND_PLUGINS_EN = [
    ...
    ("🔄 S4aUpdater - Installer", "CMD:INSTALL_S4AUPDATER"),
    ...
    ("🔄 MyUpdater v5.1 - Installer", "CMD:INSTALL_MYUPDATER"),
    ...
    ("🖼️ Picon Updater - Installer (Picons)", "CMD:INSTALL_PICON_UPDATER"),
...
DIAGNOSTICS_PL = [
    ...
    ("🔍 Szukaj w AIO Panel", "CMD:AIO_SEARCH"),
    ("⭐ Ulubione (0 = dodaj/usuń)", "CMD:AIO_FAVORITES"),
    ("🕘 Ostatnio używane", "CMD:AIO_RECENT"),
    ("🩺 Sprawdź źródła AIO", "CMD:AIO_SOURCE_HEALTH"),
    ("🧭 Kanał aktualizacji Stable/Test", "CMD:AIO_UPDATE_CHANNEL"),
    ...
    ("⏱️ Automatyczna konserwacja AIO", "CMD:SETUP_AUTO_RAM"),
    ("🧹 Wyczyść Pamięć Tymczasową", "CMD:CLEAR_TMP_CACHE"),
    ("🧹 Smart Cleanup (TMP/LOG/CACHE)", "CMD:SMART_CLEANUP"),
    ("🧹 Zwolnij pamięć Pythona AIO", "CMD:CLEAR_RAM_CACHE"),
...
DIAGNOSTICS_EN = [
    ...
    ("🔍 Search AIO Panel", "CMD:AIO_SEARCH"),
    ("⭐ Favorites (0 = add/remove)", "CMD:AIO_FAVORITES"),
    ("🕘 Recently used", "CMD:AIO_RECENT"),
    ("🩺 Check AIO sources", "CMD:AIO_SOURCE_HEALTH"),
    ("🧭 Update channel Stable/Test", "CMD:AIO_UPDATE_CHANNEL"),
    ...
    ("⏱️ Automatic AIO maintenance", "CMD:SETUP_AUTO_RAM"),
    ("🧹 Clear Temporary Cache", "CMD:CLEAR_TMP_CACHE"),
    ("🧹 Smart Cleanup (TMP/LOG/CACHE)", "CMD:SMART_CLEANUP"),
    ("🧹 Release AIO Python memory", "CMD:CLEAR_RAM_CACHE"),
...
class AIOInfoScreen(Screen):
    ...
    def _background_changelog_fetch(self):
        changelog_url = self._update_channel_urls()['changelog']
...
FUNCTION_DESCRIPTIONS = {
    "PL": {
        ...
        "🔍 Szukaj w AIO Panel": "Przeszukuje wszystkie aktualnie dostępne funkcje AIO Panel po nazwie, kategorii i identyfikatorze akcji.",
        "⭐ Ulubione (0 = dodaj/usuń)": "Otwiera zapisane Ulubione. Zaznacz dowolną funkcję w panelu i naciśnij 0, aby dodać ją lub usunąć z Ulubionych.",
        "🕘 Ostatnio używane": "Pokazuje lokalną historię ostatnio uruchamianych funkcji AIO i pozwala uruchomić je ponownie.",
        "🩺 Sprawdź źródła AIO": "Sprawdza dostępność głównych źródeł aktualizacji i zatwierdzonych instalatorów bez instalowania czegokolwiek.",
        "🧭 Kanał aktualizacji Stable/Test": "Wybiera kanał aktualizacji AIO Panel. Stable korzysta z gałęzi main; Test jest przygotowany dla przyszłej gałęzi test.",
        ...
        "⏱️ Automatyczna konserwacja AIO": "Automatyczna konserwacja AIO: porządkowanie własnych plików tymczasowych i pamięci Pythona",
        ...
        "🧹 Zwolnij pamięć Pythona AIO": "Bezpieczne uruchomienie garbage collectora Pythona; bez opróżniania cache Linuxa",
    ...
    "EN": {
        ...
        "🔍 Search AIO Panel": "Searches all currently available AIO Panel functions by name, category and action identifier.",
        "⭐ Favorites (0 = add/remove)": "Opens saved Favorites. Highlight any function and press 0 to add or remove it.",
        "🕘 Recently used": "Shows a local history of recently executed AIO actions and lets you run them again.",
        "🩺 Check AIO sources": "Checks the availability of core update endpoints and approved installers without installing anything.",
        "🧭 Update channel Stable/Test": "Selects the AIO Panel update channel. Stable uses main; Test is prepared for a future test branch.",
        ...
        "⏱️ Automatic AIO maintenance": "Automatic AIO maintenance: cleans AIO-owned temporary files and Python memory",
        ...
        "🧹 Release AIO Python memory": "Runs Python garbage collection safely; Linux filesystem cache is not dropped",
...
class PanelAIO(Screen):
    ...
    def __init__(self, session, fetched_data):
        ...
        self["act"] = ActionMap(["OkCancelActions", "ColorActions", "DirectionActions", "ChannelUpDownActions", "ChannelSelectBaseActions", "NumberActions", "HelpActions", "InfoActions"], {
            ...
            "0": self.toggle_current_favorite
        }, -1)
    ...
    def _all_action_items(self):
        """Return unique currently visible actions across all AIO tabs."""
        found = []
        seen = set()
        for tab_title, items in list(self.tabs or []):
            for item in list(items or []):
                try:
                    name, action = item
                except Exception:
                    continue
                if not action or action == 'SEPARATOR' or action in seen:
                    continue
                seen.add(action)
                found.append((ensure_unicode(name), ensure_unicode(action), ensure_unicode(tab_title)))
        return found

    def toggle_current_favorite(self):
        try:
            if getattr(self, '_focus', 'menu') != 'menu':
                return
            ...
            if not action or action == 'SEPARATOR' or action in ('CMD:AIO_FAVORITES', 'CMD:AIO_RECENT'):
                return
            added = _toggle_favorite(name, action)
            if self.lang == 'PL':
                msg = ('Dodano do Ulubionych:\n%s' % name) if added else ('Usunięto z Ulubionych:\n%s' % name)
            else:
                msg = ('Added to Favorites:\n%s' % name) if added else ('Removed from Favorites:\n%s' % name)
            show_message_compat(self.sess, msg, MessageBox.TYPE_INFO, timeout=5)
        except Exception as exc:
            _log_exception('toggle favorite', exc)
            show_message_compat(self.sess, ('Nie udało się zmienić Ulubionych.' if self.lang == 'PL' else 'Could not update Favorites.'), MessageBox.TYPE_ERROR, timeout=7)

    def open_favorites(self):
        rows = _get_favorites()
        if not rows:
            show_message_compat(self.sess, ('Brak Ulubionych. Zaznacz dowolną funkcję i naciśnij 0, aby ją dodać.' if self.lang == 'PL' else 'No Favorites yet. Highlight any action and press 0 to add it.'), MessageBox.TYPE_INFO, timeout=9)
            return
        choices = []
        for row in rows:
            name = ensure_unicode(row.get('name') or row.get('action') or '?')
            action = ensure_unicode(row.get('action') or '')
            if action:
                choices.append((name, (name, action)))
        title = '⭐ Ulubione AIO' if self.lang == 'PL' else '⭐ AIO Favorites'
        self.sess.openWithCallback(self._stored_action_selected, ChoiceBox, title=title, list=choices)

    def open_recent(self):
        rows = _get_recent()
        if not rows:
            show_message_compat(self.sess, ('Brak historii operacji.' if self.lang == 'PL' else 'No recent action history.'), MessageBox.TYPE_INFO, timeout=7)
            return
        choices = []
        for row in rows:
            name = ensure_unicode(row.get('name') or row.get('action') or '?')
            action = ensure_unicode(row.get('action') or '')
            stamp = row.get('time') or 0
            try:
                when = time.strftime('%d.%m %H:%M', time.localtime(int(stamp)))
            except Exception:
                when = '--'
            if action:
                choices.append(('%s  |  %s' % (when, name), (name, action)))
        title = '🕘 Ostatnio używane' if self.lang == 'PL' else '🕘 Recently used'
        self.sess.openWithCallback(self._stored_action_selected, ChoiceBox, title=title, list=choices)

    def _stored_action_selected(self, choice):
        if not choice:
            return
        try:
            name, action = choice[1]
            self.execute_action(name, action)
        except Exception as exc:
            _log_exception('stored action execute', exc)

    def open_aio_search(self):
        title = 'Wpisz nazwę funkcji, wtyczki lub narzędzia' if self.lang == 'PL' else 'Enter an action, plugin or tool name'
        self.sess.openWithCallback(self._aio_search_value, InputBox, title=title, text='')

    def _aio_search_value(self, value):
        query = ensure_unicode(value).strip().lower() if value is not None else ''
        if not query:
            return
        matches = []
        for name, action, tab in self._all_action_items():
            haystack = (name + ' ' + action + ' ' + tab).lower()
            if query in haystack:
                matches.append(('%s  [%s]' % (name, tab), (name, action)))
        matches = matches[:60]
        if not matches:
            show_message_compat(self.sess, ('Brak wyników dla: %s' % query if self.lang == 'PL' else 'No results for: %s' % query), MessageBox.TYPE_INFO, timeout=7)
            return
        title = ('🔍 Wyniki wyszukiwania: %s' % query) if self.lang == 'PL' else ('🔍 Search results: %s' % query)
        self.sess.openWithCallback(self._stored_action_selected, ChoiceBox, title=title, list=matches)
    ...
    def _source_probe(self, url, timeout=6):
        ...
        try:
            ...
            try:
                ...
                pass
            code = getattr(response, 'getcode', lambda: 200)() or 200
            return 200 <= int(code) < 400, 'HTTP %s' % code
        except Exception as exc:
            return False, ensure_unicode(exc)[:120]
        finally:
            try:
    ...
    def open_source_health(self):
        info = ('Sprawdzanie źródeł AIO w tle. Wynik otworzy się automatycznie.' if self.lang == 'PL' else 'Checking AIO sources in the background. Results will open automatically.')
        show_message_compat(self.sess, info, MessageBox.TYPE_INFO, timeout=4)
        thread = Thread(target=self._source_health_worker)
        try:
            thread.setDaemon(True)
        except Exception:
            pass
    ...
    def _source_health_worker(self):
        ...
        ok_count = 0
        failed = 0
        targets = _source_health_targets(include_scripts=True)
        for label, url in targets:
            ok, detail = self._source_probe(url, timeout=5)
            if ok:
                ok_count += 1
                ...
                failed += 1
                lines.append('[FAIL] %s\n       %s' % (label, detail))
        header = ('AIO Source Health Check\nSprawdzone: %s | OK: %s | Błędy: %s\n\n' % (len(targets), ok_count, failed)) if self.lang == 'PL' else ('AIO Source Health Check\nChecked: %s | OK: %s | Failed: %s\n\n' % (len(targets), ok_count, failed))
        report = header + '\n'.join(lines)
        reactor.callFromThread(self._show_source_health_report, report)

    def _show_source_health_report(self, report):
        title = '🩺 Stan źródeł AIO' if self.lang == 'PL' else '🩺 AIO source health'
        self.sess.open(AIOTextViewerScreen, title, report, '▲/▼ Scroll  OK/EXIT=Back')

    def _get_update_channel(self):
        try:
            value = config.plugins.panelaio.update_channel.value
            return value if value in ('stable', 'test') else 'stable'
        except Exception:
            return 'stable'

    def show_update_channel_menu(self):
        current = self._get_update_channel()
        choices = [
            (('Stable (zalecany)' if self.lang == 'PL' else 'Stable (recommended)'), 'stable'),
            (('Test / Beta' if self.lang == 'PL' else 'Test / Beta'), 'test'),
        ]
        title = ('Kanał aktualizacji (aktualnie: %s)' % current) if self.lang == 'PL' else ('Update channel (current: %s)' % current)
        self.sess.openWithCallback(self._set_update_channel, ChoiceBox, title=title, list=choices)

    def _set_update_channel(self, choice):
        if not choice:
            return
        value = choice[1]
        try:
            config.plugins.panelaio.update_channel.value = value
            config.plugins.panelaio.update_channel.save()
            if configfile is not None:
                configfile.save()
            msg = ('Kanał aktualizacji ustawiono na: %s.' % value if self.lang == 'PL' else 'Update channel set to: %s.' % value)
            if value == 'test':
                msg += ('\nKanał testowy zadziała po opublikowaniu gałęzi test w repozytorium.' if self.lang == 'PL' else '\nThe test channel becomes active when the test branch is published in the repository.')
            show_message_compat(self.sess, msg, MessageBox.TYPE_INFO, timeout=9)
        except Exception as exc:
            _log_exception('update channel save', exc)
            show_message_compat(self.sess, ('Nie udało się zapisać kanału aktualizacji.' if self.lang == 'PL' else 'Could not save update channel.'), MessageBox.TYPE_ERROR, timeout=8)

    def _update_channel_urls(self):
        channel = self._get_update_channel()
        branch = 'test' if channel == 'test' else 'main'
        base = 'https://raw.githubusercontent.com/OliOli2013/PanelAIO-Plugin/%s/' % branch
        return {
            'channel': channel,
    ...
    def run_with_confirmation(self):
        ...
        if self.image_type in ["hyperion", "vti"] and action == "CMD:MANAGE_DVBAPI":
            self.sess.openWithCallback(lambda ret: self.execute_action(name, action) if ret else None, MessageBox, "UWAGA (Hyperion/VTi): Opcja może nie działać.\nKontynuować?", type=MessageBox.TYPE_YESNO)
            return

        if action == 'CMD:INSTALL_S4AUPDATER':
            warning = (
                "S4aUpdater jest publikowany przez wydawcę przez starsze połączenie HTTP.\n"
                "AIO pobierze dokładnie wskazany instalator do /tmp, sprawdzi podstawową poprawność i dopiero go uruchomi.\n\nKontynuować?"
                if self.lang == 'PL' else
                "S4aUpdater is published by its provider over legacy HTTP.\n"
                "AIO will download the exact installer to /tmp, run basic validation and only then execute it.\n\nContinue?"
            )
            self.sess.openWithCallback(lambda ret: self.execute_action(name, action) if ret else None, MessageBox, warning, type=MessageBox.TYPE_YESNO)
            return

        if not _action_requires_confirmation(action):
            self.execute_action(name, action)
        else:
            question = ("Czy wykonać akcję:\n'{}'?" if self.lang == 'PL' else "Execute action:\n'{}'?").format(name)
            self.sess.openWithCallback(lambda ret: self.execute_action(name, action) if ret else None, MessageBox, question, type=MessageBox.TYPE_YESNO)
    ...
    def show_auto_ram_menu(self):
        ...
        try:
            ...
            pass
        if current == 'off':
            current_label = 'wyłączona' if self.lang == 'PL' else 'disabled'
        elif current == 'startup':
            current_label = 'przy starcie GUI' if self.lang == 'PL' else 'at GUI startup'
        elif current == '1440':
            current_label = 'raz dziennie' if self.lang == 'PL' else 'once daily'
        elif current == '4320':
            current_label = 'raz na 3 dni' if self.lang == 'PL' else 'every 3 days'
        else:
            current_label = ('tryb starszy: %s min' % current) if self.lang == 'PL' else ('legacy mode: %s min' % current)
        title = ("Automatyczna konserwacja AIO (aktualnie: {})".format(current_label) if self.lang == 'PL' else "Automatic AIO maintenance (current: {})".format(current_label))
        choices = (
            [("Wyłącz", "off"), ("Przy starcie GUI", "startup"), ("Raz dziennie", "1440"), ("Raz na 3 dni", "4320")]
            if self.lang == 'PL' else
            [("Disable", "off"), ("At GUI startup", "startup"), ("Once daily", "1440"), ("Every 3 days", "4320")]
        )
    ...
    def set_auto_ram_timer(self, choice):
        ...
        try:
            ...
            print("[AIO Panel] AIO maintenance save error:", e)

        if value == "off":
            g_auto_ram_timer.stop()
            g_auto_ram_active = False
            show_message_compat(self.sess, "Automatyczna konserwacja AIO WYŁĄCZONA." if self.lang == 'PL' else "Automatic AIO maintenance DISABLED.", MessageBox.TYPE_INFO)
        elif value == 'startup':
            g_auto_ram_timer.stop()
            g_auto_ram_active = False
            run_auto_ram_clean_task()
            show_message_compat(self.sess, ("Konserwacja zostanie wykonywana przy każdym starcie GUI." if self.lang == 'PL' else "Maintenance will run at each GUI startup."), MessageBox.TYPE_INFO)
        else:
            try:
                minutes = int(value)
                g_auto_ram_timer.start(minutes * 60000, False)
                g_auto_ram_active = True
                if value == '1440':
                    label = 'raz dziennie' if self.lang == 'PL' else 'once daily'
                elif value == '4320':
                    label = 'raz na 3 dni' if self.lang == 'PL' else 'every 3 days'
                else:
                    label = ('co %s min' % minutes) if self.lang == 'PL' else ('every %s min' % minutes)
                msg = ("Automatyczna konserwacja AIO WŁĄCZONA (%s)." % label) if self.lang == 'PL' else ("Automatic AIO maintenance ENABLED (%s)." % label)
                show_message_compat(self.sess, msg, MessageBox.TYPE_INFO)
            except Exception as e:
                print("[AIO Panel] AIO maintenance start error:", e)
                show_message_compat(self.sess, "Błąd ustawień automatycznej konserwacji AIO." if self.lang == 'PL' else "Automatic AIO maintenance configuration error.", MessageBox.TYPE_ERROR)
    ...
    def _safe_legacy_action_command(self, title, raw):
        ...
        if url_match and ('| /bin/sh' in raw or '| sh' in raw or '| /bin/bash' in raw or '| bash' in raw):
            ...
            if not _is_https_allowed(url) or not _is_trusted_remote_script(url):
                return None
    ...
    def _show_action_result(self, result):
        normalized = _normalize_action_result(result)
        try:
            if normalized.get('success'):
                AIO_LOGGER.info('ACTION result | success | code=%s', normalized.get('code'))
            else:
                AIO_LOGGER.error('ACTION result | failed | code=%s | %s', normalized.get('code'), ensure_unicode(normalized.get('details'))[-1200:])
        except Exception:
            pass
        if normalized.get('success'):
            ...
            detail = ensure_unicode(normalized.get('details') or '').strip()
    ...
    def execute_action(self, name, action):
        title = name
        try:
            AIO_LOGGER.info('ACTION start | %s | %s', ensure_unicode(name), ensure_unicode(action))
            if _should_record_recent(action):
                _record_recent(name, action)
        except Exception as exc:
            _log_exception('action history/logging', exc)
        if action.startswith('picons:'):
            ...
            if not _is_https_allowed(url) or not _is_trusted_remote_script(url) or (script_arg and not _validate_identifier(script_arg)):
                AIO_LOGGER.warning('Blocked untrusted remote installer: %s', ensure_unicode(url))
                show_message_compat(self.sess, ('Źródło instalatora nie znajduje się w Trusted Source Registry AIO.' if self.lang == 'PL' else 'Installer source is not present in the AIO Trusted Source Registry.'), MessageBox.TYPE_ERROR, timeout=12)
                return
            ...
            # AIO 16.x routes modern/user-facing actions through the modular dispatcher first.
    ...
    def install_iptv_dream_simplified(self):
        # Dedicated compatibility installer: some Enigma2 images have incomplete CA bundles.
        # Keep the requested --no-check-certificate behaviour, but download to /tmp first.
        title = "IPTV Dream - Instalator" if self.lang == 'PL' else "IPTV Dream - Installer"
        status = os.path.join(PLUGIN_TMP_PATH, 'iptv_dream_%s.status' % int(time.time() * 1000))
        script = os.path.join(PLUGIN_PATH, 'install_iptv_dream_safe.sh')
        command = '/bin/sh %s %s' % (_safe_shell_arg(script), _safe_shell_arg(status))
        run_command_in_background(self.sess, title, [command], callback_on_finish=lambda result: self._show_action_result(result))

    def install_s4aupdater_safe(self):
        # s4aupdater.one.pl publishes the official installer only over legacy HTTP.
        # AIO therefore treats this as one explicit compatibility exception: exact URL only,
        # download to a private temporary file, basic validation, hash logging, then /bin/sh.
        title = "S4aUpdater - Instalator" if self.lang == 'PL' else "S4aUpdater - Installer"
        status = os.path.join(PLUGIN_TMP_PATH, 's4aupdater_%s.status' % int(time.time() * 1000))
        script = os.path.join(PLUGIN_PATH, 'install_s4aupdater_safe.sh')
        command = '/bin/sh %s %s' % (_safe_shell_arg(script), _safe_shell_arg(status))
        run_command_in_background(self.sess, title, [command], callback_on_finish=lambda result: self._show_action_result(result))

    def install_picon_updater_safe(self):
        # Upstream command requested by the author:
        # wget -qO - https://raw.githubusercontent.com/OliOli2013/PiconUpdater/main/installer.sh | /bin/sh
        # AIO keeps the same source/options but stages and validates the script first.
        title = "Picon Updater - Instalator" if self.lang == 'PL' else "Picon Updater - Installer"
        status = os.path.join(PLUGIN_TMP_PATH, 'picon_updater_%s.status' % int(time.time() * 1000))
        script = os.path.join(PLUGIN_PATH, 'install_picon_updater_safe.sh')
        command = '/bin/sh %s %s' % (_safe_shell_arg(script), _safe_shell_arg(status))
        run_command_in_background(self.sess, title, [command], callback_on_finish=lambda result: self._show_action_result(result))

    def install_myupdater_safe(self):
        # Upstream command requested by the author:
        # wget -q -O - https://raw.githubusercontent.com/OliOli2013/MyUpdater-Plugin/main/installer.sh | sh
        # AIO stages and validates it before execution.
        title = "MyUpdater v5.1 - Instalator" if self.lang == 'PL' else "MyUpdater v5.1 - Installer"
        status = os.path.join(PLUGIN_TMP_PATH, 'myupdater_%s.status' % int(time.time() * 1000))
        script = os.path.join(PLUGIN_PATH, 'install_myupdater_safe.sh')
        command = '/bin/sh %s %s' % (_safe_shell_arg(script), _safe_shell_arg(status))
        run_command_in_background(self.sess, title, [command], callback_on_finish=lambda result: self._show_action_result(result))
